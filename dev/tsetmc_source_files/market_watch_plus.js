// source from: http://old.tsetmc.com/tsev2/res/loader.aspx?t=p&_550
...
var TemplateBorder = [
    ["", "بدون خط"],
    ["border-left:1px solid black", "سمت چپ"],
    ["border-left:1px solid black", "سمت راست"],
    ["border-left:1px solid black;border-right:1px solid black", "هر دو طرف"]
];
var TemplateColor = [
    ["#000000", "سیاه"],
    ["#990000", "قرمز"],
...
var TemplateData = [
    ...
    ["pf", "اولین قیمت", 50],
    ["pmin", "کمترین قیمت", 50],
    ["pmax", "بیشترین قیمت", 50],
    ["pl", "آخرین قیمت", 50],
    ["plc", "آخرین قیمت - تغییر", 35],
    ["plp", "آخرین قیمت - درصد", 35],
    ["pc", "قیمت پایانی", 50],
    ["pcc", "قیمت پایانی - تغییر", 35],
    ["pcp", "قیمت پایانی - درصد", 35],
    ["eps", "EPS", 35],
    ["pe", "P/E", 35],
    ["pd1", "قیمت خرید", 40],
    ["zd1", "تعداد خریدار", 50],
    ...
    var MarketWatchPlus = {
        ...
        ShowSettings: function() {
            ...
            switch (mw.Settings.ViewMode) {
                case 0:
                    ...
                    break;
                case 1:
                    html += "&nbsp;<div class='MwText' style='width:160px;height:30px'>5نمایش نمادهای معامله شده";
                    break;
                case 2:
        ...
        ExportClick: function(type) {
            switch (type) {
                ...
                case "yourmw":
                    ...
                    html = html.replace(/loader.aspx/g, "http://old.tsetmc.com/loader.aspx");
        ...
        RenderExcelData: function() {
            ...
            for (var key in mw.AllRows) {
                ...
                if (mw.Settings.Market == 2 && (flow == "1" || (flow == "3" && (yval != "320" && yval != "321" && yval != "602" && yval != "603")))) {
                    continue
                }
                ...
                if (mw.Settings.ShowOraghMosharekat == 0 && (yval == "306" || yval == "301" || yval == "706" || yval == "208" || yval == "206")) {
                    continue
                }
                ...
                if (mw.Settings.ShowEkhtiarForoush == 0 && (yval == "600" || yval == "602" || yval == "605" || yval == "603" || yval == "311" || yval == "312" || yval == "320" || yval == "321")) {
                    continue
                }
        ...
        QueryWindowFilterNames: function() {
            ...
            for (var ipos = 0; ipos < mw.Settings.Filters.length; ipos++) {
                ...
                html += "<div class='awesome " + cls + "' style='display:inline-block;width:150px' onclick=\"mw.SelectFilter(" + ipos + ");mw.ShowSettings();mw.SaveParams();mw.QueryWindowFilterNames()\" role='button' >" + mw.Settings.Filters[ipos].FilterName + "</div>"
            }
            html += "<br/><div class='awesome black' style='display:inline-block;width:150px' onclick=\"mw.QueryWindowNewFilter();mw.QueryWindowFilterNames()\" role='button' >فیلتر جدید</div>";
            mw.NotificationPermission ? cls = "" : cls = "tra";
            html += "<br/><div class='awesome " + cls + "' style='display:inline-block;width:150px' onclick=\"if(typeof Notification!='undefined')Notification.requestPermission(function(perm) {if(perm=='granted'){mw.NotificationPermission=!mw.NotificationPermission;mw.QueryWindowFilterNames()}});\" role='button' >برای نمایش هشدار در هنگام اضافه/حذف نماد کلیک کنید</div>";
            html += "</div>";
        ...
        QueryWindowEditFilter: function() {
            ...
            if (mw.Settings.FilterNo != -1) {
                ...
                html += "<div class='awesome blue' style='display:inline-block;width:100px' onclick=\"mw.QueryWindowSaveFilter()\" role='button' >ثبت</div>";
                html += "<div class='awesome ' style='display:inline-block;width:100px' onclick=\"mw.QueryWindowCheckFilter()\" role='button' >اعتبار سنجی</div>";
                html += "<div class='awesome tra' style='display:inline-block;width:50px' onclick=\"mw.QueryWindowEditFilter()\" role='button' >تصحیح</div>";
                html += "<div class='awesome red' style='display:inline-block;width:50px' onclick=\"mw.QueryWindowConfirmDeleteFilter()\" role='button' >حذف فیلتر</div>";
                html += "<div style='float:left' class='awesome' onclick='ShowHelpWindow(\"151715\",true)' role='button' >راهنمای ساخت فیلتر</div>";
                html += "<br/>";
        ...
        PrepareFilterCode: function(RawCode) {
            ...
            FilterCode = FilterCode.replace(/\x28pd4\x29/g, 'parseFloat(row["pd4"])');
            FilterCode = FilterCode.replace(/\x28zd4\x29/g, 'parseFloat(row["zd4"])');
            FilterCode = FilterCode.replace(/\x28qd4\x29/g, 'parseFloat(row["qd4"])');
            FilterCode = FilterCode.replace(/\x28po4\x29/g, 'parseFloat(row["po4"])');
            FilterCode = FilterCode.replace(/\x28zo4\x29/g, 'parseFloat(row["zo4"])');
            FilterCode = FilterCode.replace(/\x28qo4\x29/g, 'parseFloat(row["qo4"])');
            FilterCode = FilterCode.replace(/\x28pd5\x29/g, 'parseFloat(row["pd5"])');
            FilterCode = FilterCode.replace(/\x28zd5\x29/g, 'parseFloat(row["zd5"])');
            FilterCode = FilterCode.replace(/\x28qd5\x29/g, 'parseFloat(row["qd5"])');
            FilterCode = FilterCode.replace(/\x28po5\x29/g, 'parseFloat(row["po5"])');
            FilterCode = FilterCode.replace(/\x28zo5\x29/g, 'parseFloat(row["zo5"])');
            FilterCode = FilterCode.replace(/\x28qo5\x29/g, 'parseFloat(row["qo5"])');
            ...
            FilterCode = FilterCode.replace(/\x28predtran\x29/g, 'parseInt(row["predtran"],10)');
            FilterCode = FilterCode.replace(/\x28buyop\x29/g, 'parseInt(row["buyop"],10)');
        ...
        StartMarketWatch: function() {
            ...
            $(document.body).append("<div id='infop' style='direction:rtl;font-size:12px;text-align:right;border-right:2px solid black;position:fixed;top:50px;overflow:auto;bottom:0px;width:280px;left:3px;background-color:#ffffdd'><div class='MwText' style='width:100%;height:25px;margin:5px;display:flex;justify-content: right;font:normal bold 11px/22px Tahoma,sans-serif;' ><div id='userFullName'></div><br/><a id='userLink' style='margin-right:10px;'></a><a style='margin-right:10px;color:red;clear: both;' href='http://main.tsetmc.com/MarketWatch'>نسخه آزمایشی دیده بان بازار</a></div> <div id='infotab' style='text-align:center;'></div><div id='infos' class='rainbow_inf_elm'></div><div id='infoc'class='rainbow_inf_elm'><div id='RoomName' onclick='tw.ChooseRoomUI()'>نام کلاس: برای انتخاب کلاس کلیک کنید</div><input id='twsendbox' onkeypress='tw.SendBoxKeyPress(event)' class='twbox'/><div id='twviewer'/></div></div>");
            DrawRainbowTab({
                tabName: "inf",
                tabPlace: "#infotab",
                firstColor: 3,
        ...
        RenderOne: function(row) {
            ...
            rowHTML = rowHTML.replace("{qd3}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qd3) : addCommas(row.qd3));
            rowHTML = rowHTML.replace("{qo3}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qo3) : addCommas(row.qo3));
            rowHTML = rowHTML.replace("{zo4}", addCommas(row.zo4));
            rowHTML = rowHTML.replace("{zd4}", addCommas(row.zd4));
            rowHTML = rowHTML.replace("{pd4}", addCommas(row.pd4));
            rowHTML = rowHTML.replace("{po4}", addCommas(row.po4));
            rowHTML = rowHTML.replace("{qd4}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qd4) : addCommas(row.qd4));
            rowHTML = rowHTML.replace("{qo4}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qo4) : addCommas(row.qo4));
            rowHTML = rowHTML.replace("{zo5}", addCommas(row.zo5));
            rowHTML = rowHTML.replace("{zd5}", addCommas(row.zd5));
            rowHTML = rowHTML.replace("{pd5}", addCommas(row.pd5));
            rowHTML = rowHTML.replace("{po5}", addCommas(row.po5));
            rowHTML = rowHTML.replace("{qd5}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qd5) : addCommas(row.qd5));
            rowHTML = rowHTML.replace("{qo5}", mw.Settings.BigNumberSymbol == 1 ? bigNumber(row.qo5) : addCommas(row.qo5));
            rowHTML = rowHTML.replace("{cfield0}", row.cfield0);
            rowHTML = rowHTML.replace("{cfield1}", row.cfield1);
            rowHTML = rowHTML.replace("{cfield2}", row.cfield2);
            if (mw.Settings.ColorChangeEnable == 1) {
        ...
        RenderData: function() {
            ...
            if (mw.userFullName) {
                ...
                $("#userFullName").html("");
                $("#userLink").attr("href", MembersSite() + "/UserLoginnew.aspx?" + window.location.href).html("ورود")
            }
            mw.FirstTime = false
        },

...
for string in strings:
    ...
    pattern = sub(r"((\w+)\s*)=('?).*?'?([,;])", r"\1=\3(?P<\2>[^\4]*)\3\4", string)
    # LVal18AFC is defined two times!
    pattern = pattern.replace('(?P<LVal18AFC>[^,]*)', '[^,]*', 1)

    # the NAV='(?P<NAV>[^;]*)' is sometimes not present
    if pattern.endswith("NAV='(?P<NAV>[^;]*)';"):
        pattern = pattern.replace("NAV='(?P<NAV>[^;]*)';", "(NAV='(?P<NAV>[^;]*)';)?")
    else:
        pattern += "(NAV='(?P<NAV>[^;]*)';)?"
